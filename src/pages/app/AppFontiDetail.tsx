...
type AppFontiDetailProps = {
  sourceId: string;
  onBack: () => void;
};

export function AppFontiDetail({ sourceId, onBack }: AppFontiDetailProps) {
  const source = useMemo(() => sourceRows.find((s) => s.id === sourceId), [sourceId]);

  if (!source) {
    return (
      <div className="space-y-6">
        <Button variant="ghost" className="px-0 text-slate-700 hover:bg-transparent" onClick={onBack}>
          <ChevronLeft className="mr-1 h-4 w-4" /> Torna alle fonti
        </Button>
        <Card className="rounded-[28px] shadow-sm">
          <CardContent className="p-8 text-center"><p className="text-slate-600">Fonte non trovata</p></CardContent>
        </Card>
      </div>
    );
  }
      ...
      <div className="space-y-6">
        <div>
          <div className="mb-4 flex flex-wrap gap-2">
            <Badge className="rounded-full bg-slate-100 text-slate-700 hover:bg-slate-100">{source.type}</Badge>
        ...
        <div className="grid gap-6 md:grid-cols-2">
          <Card className="rounded-[28px] shadow-sm">
            <CardHeader><CardTitle className="text-sm font-semibold">Informazioni principali</CardTitle></CardHeader>
            <CardContent className="space-y-4">
              ...
              <div>
                ...
                <a href={source.url} target="_blank" rel="noopener noreferrer" className="mt-1 inline-flex items-center gap-2 text-sm text-blue-600 hover:text-blue-700">
          ...
          <Card className="rounded-[28px] shadow-sm">
            <CardHeader><CardTitle className="text-sm font-semibold">Status di monitoraggio</CardTitle></CardHeader>
            <CardContent className="space-y-4">
              <div>
              ...
              <div>
                ...
                <Badge variant="outline" className="mt-2 rounded-full">{source.priority}</Badge>
              </div>
              <div>
        ...
        <Card className="rounded-[28px] shadow-sm">
          <CardHeader><CardTitle className="text-sm font-semibold">Focus tematici monitorati</CardTitle></CardHeader>
          <CardContent>
            <div className="flex flex-wrap gap-2">
              {source.focus.map((topic) => (
                <Badge key={topic} variant="outline" className="rounded-full">{topic}</Badge>
              ))}
            </div>
          </CardContent>
        </Card>
